<?php
// ============================================================
//  admin/allotment.php  –  Allotment List
//  Lists students who have been cleared by Finance (status = 4)
//  and are ready for final allotment.
// ============================================================
...
// Allow super_admin and specific allotment roles
...
// Fetch students with status = 4 (Finance Cleared)
$stmt = $pdo->prepare("
    SELECT id, uan_no, cname, programme_name, department_name, admitted_category, enrolment_no, admission_date
...
// Get count of pending allotments
$countStmt = $pdo->query("SELECT COUNT(*) FROM admitted_students WHERE status = 4");
...
<title>Allotment – ASU Portal</title>
...
.status-badge{display:inline-block;background:#8B5CF6;color:#fff;padding:4px 14px;border-radius:20px;font-size:12px;font-weight:600;}
...
.finance-cleared{display:inline-flex;align-items:center;gap:4px;padding:3px 10px;border-radius:12px;font-size:11px;font-weight:600;background:#edfdf5;color:#1a6640;}
.finance-cleared::before{content:"✓";font-weight:800;}

.btn-proceed{padding:7px 18px;border-radius:8px;font-size:12.5px;font-weight:600;border:none;cursor:pointer;font-family:'Inter',sans-serif;background:linear-gradient(135deg,#8B5CF6,#6D28D9);color:#fff;text-decoration:none;display:inline-block;transition:opacity .15s,transform .15s;}
...
<main class="main">
  <div class="page-header">
    <div>
      <div class="page-title">Final Allotment</div>
      <div class="page-sub">Students cleared by Finance, ready for final admission allotment</div>
    </div>
  ...
  <div class="stats-row">
    <div class="stat-card">
      ...
      <div class="stat-lbl">Pending Allotment</div>
    </div>
  </div>
        ...
        <tbody>
          ...
          <tr class="no-results">
            <td colspan="8">
              ...
              No students currently pending final allotment.
            </td>
          </tr>
          ...
          <tr>
            ...
            <td><span class="finance-cleared">Finance Cleared</span></td>
            <td style="text-align:right">
              <a class="btn-proceed" href="allotment_review.php?id=<?= (int)$s['id'] ?>">Proceed →</a>
            </td>
          </tr>
